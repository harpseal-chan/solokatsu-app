...
export const initialSoloKatsuData: Prisma.SoloKatsuCreateInput[] = [
  {
    ...
    impressions:
      `革張りのソファに身を委ねて、珈琲の香りが漂うレトロな空間で読書をしたりアンティークな雑貨を眺めて過ごしています。  
私のお気に入りは、純喫茶ならではのスイーツを味わうこと。特に、昔ながらの硬めのプリンと鮮やかなクリームソーダの甘い組み合わせは心がときめきます。`.trim(),
    tips: `
...
- 店内は喫煙可能な場合もあるので、煙が気になる方は口コミ等で確認しておきましょう。
  ...
  },
  {
    ...
    impressions:
      `目の前の土の感触と成形に意識を集中させるため、自然とデジタルデトックスになります。  
難しい箇所は専門スタッフが丁寧に手伝ってくれるコースを選べば、初心者でも安心して参加できます。  
焼き上がった器は後日自宅に届き、食卓に並ぶたびに愛着が湧いてきます`.trim(),
    tips: `
...
- 土をこねる際に邪魔になるため、爪は事前に短く切っておくのがおすすめです。
  ...
  },
  {
    ...
    impressions:
      `私は動物が好きで、猫・フクロウ・ハリネズミなど色んな動物カフェに行きました。  
普段はなかなか出会えない珍しい動物たちとも触れ合うことができてとても癒やされます。  
周囲の人たちも目の前の動物に夢中なので、一人で来ても目立ちません。`.trim(),
    tips: `
...
- 服に毛やよだれが付く場合があるため、汚れても良く、引っかかりにくい素材の服を選ぶのがおすすめです。
  ...
  },
  {
    ...
    impressions:
      `国内にいながら異国情緒を感じ、手軽に世界の料理を味わうことができます。  
初心者は、店舗数が多くて入りやすいインドカレーなどの身近なジャンルから始めていって、慣れてきたらより本格的で珍しい国のお店に挑戦してみるのもおすすめです。  
その国の食文化に触れることで、何が主食なのか、どんなスパイスが使われているのかなども学ぶことができ、まるで旅をしているような知的好奇心も満たされます。`.trim(),
    tips: `
- メニュー名だけではどんな料理か想像しにくい場合もあるので、事前にSNSやグルメサイトでレビューをチェックしておくと注文しやすいです。
  ...
  },
  {
    ...
    impressions:
      `ソロ活の定番スポットで、カウンター席中心のお店が多いのでおひとり様にもやさしいです。  
お店ごとに具材やスープに特色があり、こだわりを感じる一杯をじっくりと味わいます。  
写真は個人的おすすめのラーメンで、「卍力（まんりき）」の「スパイス・ラー麺」です。複雑なスパイスや酸味の効いたスープ、清涼感のあるパクチーの香り、ほろほろのチャーシューが絶品で、また食べたくなる中毒性のある味です。`.trim(),
  },
  {
    title: "映画館",
  ...
  },
  {
    ...
    impressions:
      `誰かと一緒だと相手のペースを気にしてしまいますが、ソロだとマイペースで鑑賞できるのが魅力です。  
    一つの作品を納得いくまでじっくり眺めてもよし、興味のないエリアをスキップしてもよし。  
    自分の感性だけに集中してアートと向き合う特別な時間を過ごせます。`.trim(),
    tips: `
...
省スペースで手軽に始められて、日々の成長を観察する楽しさがあるソロ活です。`.trim(),
    tips: `
...
イカ墨は歯が黒くなるので二人以上だと注文に躊躇いますが、一人だとあまり気にしなくていいので楽です。`.trim(),
  },
  {
    ...
    impressions:
      `好きな作品のシンフォニックコンサートが開催されると知り、早速チケットを取って1人で会場へ足を運びました。  
いざ開演すると、ステージから押し寄せる楽器の音圧と美しい旋律にただただ圧倒されました。原曲の良さはそのままに、お洒落なジャズアレンジで紡がれる生演奏を耳にした瞬間は、一気にテンションが上がりました。  
終演後も感動が収まらず、しばらく1人で心地よい余韻に浸っていました。`.trim(),
  },
  {
    title: "料理教室",
    description:
  ...
  },
  {
    ...
    impressions:
      `お金をかけずに気軽にできるソロ活です。近所であっても、普段は通らない道をあえて選んで歩いてみると、新しいお店や綺麗な景色などの新鮮な発見があったりします。  
お気に入りの音楽やオーディオブックを聴きながら、自分のペースで気の向くままに歩く時間は、いい気分転換になります。`.trim(),
    tips: `
- 歩くとのどが渇いて自販機で飲み物を買いがちですが、お気に入りの水筒に好きな飲み物を入れて持っていくと、節約になりテンションもあがります。
  ...
  },
  {
    ...
    impressions:
      `フルーツパーラーとは、新鮮な果物を使ったパフェやフルーツサンド、ジュースなどのスイーツや軽食を楽しめる喫茶店のことです。  
...
`.trim(),
  ...
  },
  {
    ...
    impressions:
      `使わなくなったモノは捨てずにフリマアプリに出品すると売れるかもしれません。  
服や本はもちろん、トイレットペーパーの芯など意外なものも売れます。  
商品の写真撮影や説明文の作成、梱包などやることは多く最初は面倒ですが、慣れると簡単です。  
家の中がすっきりと片付くだけでなく、臨時収入にもなるので一石二鳥です。`.trim(),
    tips: `
...
- 最近のフリマアプリは、匿名で簡単に発送できるシステムが整っているため、プライバシーを守りながら手軽に利用できます。
  ...
  },
  {
    ...
    impressions:
      `どのメニューを頼んでもハズレがなく、お腹いっぱい食べてもお財布に優しい、安定して満足度の高いコスパ最強レストランです。  
...
今度は無料の調味料でメニューのアレンジにも挑戦してみたいです。`.trim(),
  },
  {
    title: "メイクレッスン",
    description:
  ...
  },
  {
    ...
    impressions:
      `徒歩0分で素晴らしい絶景を見ることができます。神奈川県横浜市にある、海の見える駅として有名なJR鶴見線の海芝浦駅に行きました。  
電車を降りてホームの目の前が海というロケーションで、遠くには横浜ベイブリッジを望むことができます。  
駅構内には公園が併設されており、折り返しの電車が出発するまでの間、ベンチに座りながら海をぼーっと眺める時間は最高に贅沢なひとときです。`.trim(),
    tips: `
...
- 電車の本数が少なく、終電の時間帯は早いため、事前に時刻表をしっかりと確認しておきましょう。
  ...
  },
  {
    ...
    impressions:
      `ホットケーキミックスに牛乳・卵・サラダ油といった基本の材料を混ぜて焼くだけで、様々なスイーツが手軽に作れます。  
特にパウンドケーキやスコーンは工程が少なく、チョコやナッツなどの具材で自由にアレンジもできます。`.trim(),
    tips: `
- YouTubeチャンネル「べるっぱのホットケーキミックス研究所」で、簡単に作れるスイーツがたくさん紹介されているので参考にしてみてほしいです。
  ...
  },
  {
    ...
    budget: Budget.FREE,
    impressions:
      `敷居が高く見えますが、予約不要・無料で入場可能です。玄関ホールの開廷表ファイルを参考に興味のある裁判を選びます。  
初めて行く場合は、事件の概要が分かりやすい刑事事件の初公判（開廷表に「新件」と記載されたもの）を傍聴するのがおすすめです。  
法廷内は私語厳禁で誰もが裁判の行方に集中しているため、一人で席に座っていても周囲の目は全く気になりません。`.trim(),
    tips: `
...
- 裁判所の敷地内および法廷内での写真動画撮影、録音は一切禁止されています。（紙のメモはOK）`,
  ...
  },
  {
    ...
    impressions:
      `以前から少し気になっていた「シーシャバー」に初挑戦してみました。シーシャとは水タバコのことで、フレーバーが付けられたタバコの葉を炭で燻らし、その煙を一度水に通して吸う嗜好品です。  
私はお店おすすめのブルーベリー&アールグレイフレーバーを注文しました。運ばれてきたシーシャを吸い込んで、ポコポコという水の音を聞きながら心地よい香りの煙をふわっと吐き出すと想像以上にリラックスできます。
`.trim(),
    tips: `
- お店を選ぶときは、店内に適度な解放感や窓、換気設備がしっかりあるかチェックしておくと安心です。
- 長時間吸っていると軽い酸欠で頭痛が起きる可能性があるので、セットのドリンクなどで定期的に水分補給をしましょう。
`,
  },
  {
    title: "部屋の模様替え",
  ...
  },
  {
    ...
    impressions:
      `日々の忙しさから少し離れたいなと思い、癒しを求めてレトロな雰囲気の金魚釣り堀へ行ってみました。  
お金を払って釣り竿と餌をもらい、池の周りにある椅子に座ります。  
最初は音沙汰なしでしたが、常連っぽい人の動きを真似て竿を揺らしてみると綺麗な金魚がピチピチと釣れました。  
ただ浮きの動きだけに集中したり、釣れた金魚の泳ぐ姿を観察する時間は、驚くほど心が静まり、最高の癒やしになりました。`.trim(),
  },
  {
    title: "会話禁止カフェ",
    description:
...
ドリンク1杯あたりの滞在時間が決まっているお店も多く、誰にも気を使うことなく、安心して一人の時間を満喫できます。`.trim(),
    tips: `
...
`,
  ...
  },
  {
    ...
    impressions:
      `「夜パフェ（シメパフェ）」とは、札幌を中心に、はしご酒の後にパフェを食べるという文化が発祥です。  
店内はバーのような落ち着いた雰囲気で、繊細なデザインの焼き菓子やアルコールの効いたソルベが使われたパフェが多く、甘すぎない大人な味わいが魅力です。  
お酒を飲んだ後ではなくても、仕事帰りに疲れたときのご褒美としてパフェを食べるのはどうでしょう。`.trim(),
    tips: `
...
`,
  ...
  },
  {
    ...
    impressions:
      `今後やりたい仕事は何なのか悩んでいたときに、『世界一やさしい「やりたいこと」の見つけ方 人生のモヤモヤから解放される自己理解メソッド』（八木 仁平:著）という本に出会いました。  
本に書かれている質問に答える形で、自分の「大事なこと（価値観）」「得意なこと（才能）」「好きなこと（情熱）」をノートに書き出していくと、頭の中のモヤモヤがすっきりと整理されていきます。  
やりたいことがない場合は「やりたくないこと・嫌いなこと」を書き出すのもいいですよ。`.trim(),
    tips: `
...
`,
  ...
  },
  {
    ...
    impressions:
      `造幣局とは貨幣（1円〜500円玉）や金属工芸品などを製造している日本の行政執行法人のことで、大阪市に本局、さいたま市と広島市に支局があります。  
併設の博物館では古銭や勲章の展示を見ることができ、さらに事前申し込みが必要ですが無料で工場見学も行っており、貨幣の製造工程が見られます。  
他にも個人的におすすめなのが4月に開催される大阪本局の桜の通り抜け。140品種331本の桜が咲き誇り圧巻の景色です。`.trim(),
    tips: `
...
`,
  ...
  },
  {
    ...
    budget: Budget.LIGHT,
    impressions:
      `たこ焼き器や具材の用意は少し手間がかかりますが、焼くのは意外と簡単。  
タコやネギなどの定番具材以外にはキムチやチーズもおすすめです。ソースマヨだけだと飽きるので味変で出汁につけると明石焼き風で食べられます。  
余っても冷凍できるので、たくさん作っても大丈夫です。  
誰の目も気にせず、焼き立て熱々のたこ焼きを頬張り炭酸ジュースで流し込むのが至高です。`.trim(),
  },
  {
    title: "焼肉ライク",
    description:
  ...
  },
  {
    ...
    budget: Budget.LUXURY,
    impressions:
      `自分の部屋をもっと居心地の良い場所にしたいと思い立ち、自宅のスマートホーム化に挑戦しました。  
...
`.trim(),
  },
  {
    ...
    impressions:
      `私は季節限定スイーツが好きで、お菓子が販売される時期が近づくとSNSでチェックしています。  
特にチョコミントには目がなく、6月頃になるとコンビニや無印良品で販売されるスイーツを買い占めたり、ファミレスやカフェのチョコミントフェアのメニューを楽しんだりしています。`.trim(),
  },
  {
    title: "同人誌即売会",
    description:
    ...
    impressions:
      `個人が自由に作った同人誌が集まる日本最大級のイベント、コミケにはいつも参加しています。  
なかでも評論系（趣味や仕事など幅広い知識が集まる分野）というジャンルが好きで、いつも料理や旅行、面白い体験談などの同人誌を買うことが多いです。  
商業誌では絶対に読めないような、個人の並々ならぬ情熱やニッチなこだわりが凝縮された一冊に出会えます。`.trim(),
    tips: `
...
`,
  },
  {
    title: "はとバスツアー",
    description:
      "黄色い大きなバスに乗り込み、プロのガイドや車窓からの景色に身を委ねて東京の名所を効率よく巡る",
    category: Category.ENTERTAINMENT,
    difficulty: Difficulty.NORMAL,
    budget: Budget.STANDARD,
    impressions:
      `2階建てオープンバスに乗り込み、東京タワーや銀座、レインボーブリッジなど東京の名所をめぐるツアーに参加しました。  
屋根のない2階から見る高層ビルや東京タワーは大迫力で、まるでアトラクションに乗っているかのようなワクワク感があります。  
心地よい風を全身に浴びながらガイドの解説に耳を傾けることで、知っているようで知らなかった東京の歴史や街の魅力を再発見しました。`.trim(),
    tips: `
- オープンバスは遮るものがないため、日中の場合は日焼け止めや紐付き帽子があるといいです。
`,
  },
  {
    title: "レンタサイクル",
  ...
  },
  {
    title: "日帰り旅行",
    description:
      "特急に乗って車窓からの景色を眺めながら移動し、現地のグルメや温泉を好きなペースで満喫してその日のうちにサクッと帰る",
    category: Category.ENTERTAINMENT,
    difficulty: Difficulty.NORMAL,
    budget: Budget.STANDARD,
    imageUrl: "/images/express_train.webp",
  },
  {
    title: "凧揚げ",
    description:
    ...
    budget: Budget.LIGHT,
    impressions:
      `少し目立つので万人にはおすすめできませんが、やってみると割と楽しいので入れてみました。  
...
`,
  },
  {
    title: "セルフネイル",
    description:
      "自分の好きな色やデザインを自由に選び、自宅にいながらサロン帰りのような指先を叶える手軽なおしゃれ",
    category: Category.INDOOR,
    difficulty: Difficulty.EASY,
    budget: Budget.LIGHT,
    imageUrl: "/images/nail_art.webp",
    impressions:
      `ネイルサロンだと単色でも最低5000円ぐらいかかりますが、自宅でセルフネイルであれば価格をおさえられるのが最大の魅力です。  
最初は失敗して見た目が悪くなることもありましたが、練習を重ねるごとに綺麗に塗れるようになり、長持ちするようになりました。  
サロンの予約や移動の手間もなく、ちょっとした隙間時間に自分のペースで完結できるのも気楽で気に入っています。  
綺麗に仕上がった指先がふと目に入るたびにモチベーションが上がります`.trim(),
    tips: `
- 100均でほとんどのネイルグッズを揃えられるため、初期費用をかけずに手軽にスタートできます。
- 最近は凝ったデザインのジェルネイルシールもあるので、時短でサロン級のクオリティに仕上げたいときにおすすめです。
`,
  },
  {
    title: "アクセサリー作り",
    ...
    impressions:
      `なにかクリエイティブなことがしたいけど不器用でセンスもないので、ビーズ刺繍のブローチキットを購入しました。  
説明書通りにビーズを一つひとつ縫い付けていくだけで、想像以上にいい作品が出来上がっていく過程がとても楽しいです。  
パーツや色合わせが最初から決まっているキットなら、初心者でも迷わず安心して進められます。  
縫うのに没頭しているうちに時間が経つのを忘れてしまい、自分で作ったブローチには特別な愛着が湧きました。`.trim(),
  },
  {
    title: "お菓子の工場直売所",
    description:
  ...
  },
  {
    ...
    impressions:
      `ニードルフェルトは、ふわふわの羊毛を専用の針で刺して形を作っていく手芸のことです。  
...
黙々と針を刺す作業に没頭できるので、デジタルデトックスにもなります。`.trim(),
  },
  {
    title: "生成AIの活用",
    description:
  ...
  },
  {
    ...
    impressions:
      `昔絵本で見たお菓子の家が印象に残っていて、自分も作ってみたいなと思い、森永の公式レシピを参考に作ってみました。  
溶かしたチョコを接着剤にしてビスケットで壁、チョコ菓子で屋根を組み立てていき、最後に金平糖やラムネでデコレーションしました。  
見た目も可愛く、味も市販のお菓子を使っているので美味かったです。`.trim(),
  },
  {
    title: "カルディでプチ贅沢",
    description:
  ...
  },
  {
    ...
    impressions:
      `休日にじっくり時間をかけて料理を作り、自分自身へのおもてなしをするのはいかがでしょうか。  
最近貧血気味だったので、鉄分豊富なレバーペーストを作ってみました。レバーとハツを切り分け、玉ねぎとハーブで炒め、生クリームやバターとミキサーで混ぜて……といった数々の工程を実施し、ついに完成。  
バゲットにたっぷり付けていただきました。お金も時間もかかりましたが、お店クオリティの味に仕上がり、美味しかったので作ったかいがありました。`.trim(),
  },
  {
    title: "採石場跡",
    description:
...
- 階段が多い場所なので、歩き慣れたスニーカーで行くのがおすすめです。
  ...
  },
  {
    ...
    budget: Budget.FREE,
    impressions:
      `いつか行ってみたいお店やSNSで見かけた絶景スポットを忘れないように、Googleマップにリスト化しています。  
...
ふと一人の時間ができたときも、このマップを開けば「ここから一番近い行きたい場所」がすぐに見つかるので、休日のお出かけのフットワークが軽くなりました。`.trim(),
  },
];
